...
<head>
    ...
    <title>Category Record - Ready Shopping</title>
    <!--------- Begin Css Style  --------->
                                ...
                                <div class="card">
                                    <div class="card-header">
                                        ...
                                        <div class="row right-15">
                                            <div class="col-12"> <a href="{{ url('create-category') }}" class="btn btn-primary" data-toggle="tooltip" title="Add Record!"><i class="fas fa-plus mr-2"></i>Add Category Name</a>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="card-body">
                                        <form class="form-horizontal" role="form" method="get" action="{{ url('/category') }}">
                                            <div class="row">
                                            ...
                                            <table class="table table-striped" id="table-1">
                                                ...
                                                <tbody>@if(count($datarecords) > 0)
                                                    ...
                                                    <tr>@if($pageOrder == "ASC")
                                                        ...
                                                        @endif
                                                        <td width="10%"><a href="{{ asset('public/assets/img/category/'.$data->cat_picture) }}" target="_blank"><img src="{{ asset('public/assets/img/category/'.$data->cat_picture) }}" align="center" class="center" style="width: 100px;height: 50px;padding: 5px;"></a></td>
                                                        <td>{{ $data->mac_title}}</td>
                                                        <td>{{ $data->cat_name }}</td>
                                                        <td>
                                                            <div class="material-switch" style="margin-left: -15px;">
                                                                <input id="switch-primary-{{$data->cat_id}}" value="{{$data->cat_id}}" name="toggle" type="checkbox" {{ $data->cat_status === 0 ? 'checked' : '' }}>
                                                                <label for="switch-primary-{{$data->cat_id}}" class="btn-success"></label>
                                                            </div>
                                                        </td>
                                                        <td class="text-center" width="15%">
                                                            <!--------- Begin Edit Link --------->
                                                            <a href="{{ url('edit-category',$data->cat_unique_id) }}" class="btn btn-icon btn-primary mr-1" data-toggle="tooltip" title="Edit Record!"><i class="far fa-edit"></i>
                                                            </a>
                                                            <!--------- End Edit Link --------->
                                                            <!--------- Begin Delete Link --------->
                                                            <a href="{{ url('delete-category',$data->cat_unique_id) }}" onclick="return confirm('Are you sure you want to delete this record?')" data-toggle="tooltip" title="Delete Record!">
 ...
 $('input[name=toggle]').change(function(){
    ...
    $.ajax({
      ...
      success:function(data)
      {
        if(data.status == "true")
        {
          alert("Category name active successfully.");
        }
        else if(data.status == "false")
        {
          alert("Category name inactive successfully.");
        }
      }
    });
    }); 
