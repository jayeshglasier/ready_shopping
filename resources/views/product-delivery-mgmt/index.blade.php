...
<head>
    ...
    <title>Village Record - Ready Shopping</title>
    <!--------- Begin Css Style  --------->
                                ...
                                <div class="card">
                                    <div class="card-header">
                                        ...
                                        <div class="row right-15">
                                            <div class="col-12"> <a href="{{ url('create-villages') }}" class="btn btn-primary" data-toggle="tooltip" title="Add Village Name!"><i class="fas fa-plus mr-2"></i>Add Village Name</a>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="card-body">
                                        <form class="form-horizontal" role="form" method="get" action="{{ url('/villages') }}">
                                            <div class="row">
                                                ...
                                                <div class="col-12 col-md-2 filter-forms" style="padding-right: 0px;padding-left: 10px;">
                                                    <div class="form-group">
                                                        <select class="form-control selectric" name="sort_by" required>
                                                            <option value="vil_id" @if($sortBy=='vil_id' ) selected="true" @endif>Sort By Sr.No</option>
                                                        </select>
                                                    </div>
                                                </div>
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-striped" id="table-1">
                                                ...
                                                <tbody>@if(count($datarecords) > 0)
                                                    ...
                                                    <tr>@if($pageOrder == "ASC")
                                                        ...
                                                        <td>
                                                            <div class="material-switch" style="margin-left: -15px;">
                                                                <input id="switch-primary-{{$data->vil_id}}" value="{{$data->vil_id}}" name="toggle" type="checkbox" {{ $data->vil_status === 0 ? 'checked' : '' }}>
                                                                <label for="switch-primary-{{$data->vil_id}}" class="btn-success"></label>
                                                            </div>
                                                        </td>
                                                        <td class="text-center" width="15%">
                                                            <!--------- Begin Edit Link --------->
                                                            <a href="{{ url('edit-villages',$data->vil_id) }}" class="btn btn-icon btn-primary mr-1" data-toggle="tooltip" title="Edit Village Name!"><i class="far fa-edit"></i>
                                                            </a>
                                                            <!--------- End Edit Link --------->
                                                            <!--------- Begin Delete Link --------->
                                                            <a href="{{ url('delete-villages',$data->vil_id) }}" onclick="return confirm('Are you sure you want to delete this record?')" data-toggle="tooltip" title="Delete Village Name!">
                                                    ...
                                                    @endforeach 
                                                    @else
                                                    <td colspan="4" style="text-align: center;">Record not available!</td>
                                                    @endif
                                                </tbody>
                                            </table>
 ...
 $('input[name=toggle]').change(function(){
    ...
    $.ajax({
      ...
      success:function(data)
      {
        if(data.status == "true")
        {
          alert("Village name active successfully.");
        }
        else if(data.status == "false")
        {
          alert("Village name inactive successfully.");
        }
      }
    });
    }); 
