...
class GirlBeautyController extends Controller
{
    ...
    public function index()
    {
        try
        {
            ...
            if($search)
            {   
                $data['datarecords'] = Products::orderBy('pod_id',$pageOrder)->where('pod_main_cat_id',14)->where('pod_pro_name','like','%'.$search.'%')->paginate($pages);
            }else{
                $data['datarecords'] = Products::join('tbl_categorys','tbl_products.pod_cat_id','=','tbl_categorys.cat_id')->leftjoin('tbl_sub_categorys','tbl_products.pod_sub_cat_id','=','tbl_sub_categorys.sub_id')->where('pod_main_cat_id',14)->where('pod_seller_id',0)->orderBy('pod_id',$pageOrder)->paginate($pages);
            }
            return view('admin.shop.girl-beauty-mgmt.index',$data);
        } catch (\Exception $e) {
    ...
    {
        $data['categorys'] = Category::where('cat_main_id',14)->where('cat_name','<>','')->orderBy('cat_name','ASC')->get();
        $data['choosetype'] = ChooseType::select('coo_id','coo_name')->where('coo_name','<>','')->orderBy('coo_id','ASC')->get();
        return view('admin.shop.girl-beauty-mgmt.create',$data);
    }
    ...
    public function edit($id)
    {
        ...
        return view('admin.shop.girl-beauty-mgmt.edit',$data);
    }
    ...
    public function view($id)
    {
        ...
        return view('admin.shop.girl-beauty-mgmt.view',$data);
    }
    ...
    public function store(Request $request)
    {
        ...
        {  
            ...
            $insertData['pod_colour'] = $request->pod_colour ? $request->pod_colour:'';
            ...
            if($insertData)
            {
                Session::flash('success', 'Girl Beauty details created!');
                return redirect()->intended('/girl-beauty');
            }else{
                Session::flash('error', "Girl Beauty details isn't created!");
                return redirect()->intended('/girl-beauty');
            }
    ...
    public function update(Request $request)
    {
        ...
        {  
            ...
            $updateData['pod_colour'] = $request->pod_colour ? $request->pod_colour:'';
            ...
            if($infoUpdate)
            {
                Session::flash('success', 'Girl Beauty details updated!');
                return redirect()->intended('/girl-beauty');
            }else{
                Session::flash('error', "Girl Beauty details isn't updated!");
    ...
    public function exportPdf()
    {
        ...
        $pdf = PDF::loadView('admin.shop.girl-beauty-mgmt.pdf-file', $data);
