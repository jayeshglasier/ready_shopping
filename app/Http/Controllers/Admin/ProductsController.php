...
class ProductsController extends Controller
{
    ...
    public function index()
    {
        try
        {
            ...
            if($search)
            {   
                $data['datarecords'] = Products::orderBy('pod_id',$pageOrder)->where('pod_pro_name','like','%'.$search.'%')->whereIn('pod_main_cat_id',[1,2,3,4,5,6])->paginate($pages);
            }else{
                $data['datarecords'] = Products::join('tbl_categorys','tbl_products.pod_cat_id','=','tbl_categorys.cat_id')->leftjoin('tbl_sub_categorys','tbl_products.pod_sub_cat_id','=','tbl_sub_categorys.sub_id')->where('pod_seller_id',0)->whereIn('pod_main_cat_id',[1,2,3,4,5,6])->orderBy('pod_id',$pageOrder)->paginate($pages);
            }
    ...
    {
        $data['mainCategorys'] = MainCategorys::where('mac_status',0)->whereIn('mac_id',[1,2,3,4,5,6])->get();
    ...
    public function store(Request $request)
    {
        ...
        {  
            ...
            $insertData['pod_weight'] = $request->pod_weight ? $request->pod_weight:'';
    ...
    {
        $data = SubCategory::where('sub_cat_id',$request->category_id)->where('sub_status',0)->orderby('sub_cat_name','ASC')->distinct()->get(['sub_id','sub_cat_name']);
        return $data;
    }
