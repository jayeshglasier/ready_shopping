...
class SellersController extends Controller
{
    ...
    public function index()
    {
        try
        {
            ...
            if($search)
            {   
                $data['datarecords'] = User::orderBy('id',$pageOrder)->where('use_role',2)->where('use_full_name','like','%'.$search.'%')->paginate($pages);
            }else{
                $data['datarecords'] = User::where('use_role',2)->orderBy('id',$pageOrder)->paginate($pages);
            }
            return view('sellers-mgmt.index',$data);
        } catch (\Exception $e) {
    ...
    {   
        ...
        return view('sellers-mgmt.create',$data);
    }
    ...
    public function edit($id)
    {
        ...
        return view('sellers-mgmt.edit',$data);
    }
    ...
    public function store(Request $request)
    {
        ...
        {  
            ...
            {
                Session::flash('error', 'Seller already exists!');
                return redirect()->intended('/create-sellers');
            }
        ...
        // *********************** End Default Store Dispaly Id ******************
            ...
            if($insertData)
            {
                Session::flash('success', 'Seller is created!');
                return redirect()->intended('/sellers');
            }else{
                Session::flash('error', "Seller isn't created!");
                return redirect()->intended('/sellers');
            }
    ...
    {   
        ...
        {  
            ...
            if($infoUpdate)
            {
                Session::flash('success', 'Seller updated!');
                return redirect()->intended('/sellers');
            }else{
                Session::flash('error', "Seller isn't updated!");
                return redirect()->intended('/sellers');
            }
        }catch (\Exception $e) {
             Exceptions::exception($e);
    ...
    { 
        if(User::where('id',$id)->exists())
        {
            ...
            Session::flash('success', 'Seller is deleted!');
            return redirect()->intended('/sellers');
        }else{
          Session::flash('error', "Seller isn't deleted!");
          return redirect()->intended('/sellers');
        }
    }
    ...
    public function exportPdf()
    {
        ...
        $pdf = PDF::loadView('sellers-mgmt.pdf-file', $data);
